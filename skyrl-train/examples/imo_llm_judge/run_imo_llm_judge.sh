...
if [[ -f "$ENV_FILE" ]]; then
  ...
  # Required: OPENAI_API_KEY (for the judge)
  # Optional: WANDB_API_KEY (if LOGGER=wandb)
  echo "Warning: $ENV_FILE not found; using exported environment variables instead." 1>&2
fi

# GPT-OSS: flash-attn is incompatible with attention sinks; SkyRL patches GPT-OSS to use FlexAttention
# automatically when transformers>=4.56.2, but we still explicitly disable flash-attn here.
...
uv run --isolated --extra vllm "${ENV_FILE_ARGS[@]}" -m examples.imo_llm_judge.main_imo_llm_judge \
  ...
  environment.skyrl_gym.imo_llm_judge.model="gpt-4o-mini" \
